...
const char* fallback_help_text() {
    return
        ...
        "  backend session transport directory in the kdebug state root. File transport v2\n"
        ...
        "Logs\n"
        "----\n"
        "  State defaults to ~/.kdebug. Set KDEBUG_HOME to an absolute directory to\n"
        "  isolate frontend/engine work, logs, session files, and registries.\n"
        "  Public logs live under <state-root>/sessions/<session_prefix>_<hash>/logs/actions.ndjson.\n"
        "  Stdio-loop protocol logs live beside actions.ndjson as stdio.ndjson.\n"
        "  Engine lifecycle/transport/crash logs live under <state-root>/engine/sessions/.\n"
