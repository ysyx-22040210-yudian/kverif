...
只允许工具组进入修复队列、并跳过 GLM 的 matrix 参数为：

```bash
--models gpt-5.5,qwen3.6-35b --groups with_kdebug
```

`run_matrix.sh` 会先输出 `MATRIX CONFIG`，随后逐 case 真实采集并校验 KDebug
manifest；在这些步骤全部通过前不会调用模型 API。
